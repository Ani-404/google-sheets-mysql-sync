...
async function upsertRow(table, row) {
  const p = getPool();
  if (!row.__id) throw new Error('__id is required');
...
async function fetchUnprocessedChangeLogs(limit = 50) {
  ...
  const [rows] = await p.query(
    `SELECT * FROM change_log WHERE processed=FALSE ORDER BY created_at LIMIT ?`,
    [limit]
  );
  return rows;
}
...
async function markChangeProcessed(id) {
  const p = getPool();
  return p.query(
    `UPDATE change_log SET processed=TRUE, processed_at=CURRENT_TIMESTAMP WHERE id=?`,
    [id]
  );
}
